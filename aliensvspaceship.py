...
import random
from assets import *
from sprites import Fighter, Alien, Ground
...
def start():
    global fighter, alien_speed, bomb_rate, shot_limit, score, level, stage

    fighter = Fighter(480, 540)
...
def display_end_screen(screen):
    ...
    sample_text = FONT_SM.render("Press SPACEBAR to restart game.", True, WHITE)
...
def display_stats(screen, score, level, high_score, shield):
    socre_text = FONT_SH.render("Score: " + str(score), True, WHITE)
    highscore_text = FONT_SH.render("High score: " + str(high_score), True, WHITE)
...
while not done:
        ...
        if event.type == pygame.QUIT:
            ...
            if stage == START:
                ...
                if event.key == pygame.K_SPACE and len(bullets) < shot_limit:
                    SHOT.play()
                    fighter.shoot(bullets, -bullet_speed)
                    score -= 1
    ...
    if stage == DELAY:
        ...
        for b in bombs:
            ...
            if b.intersects(fighter):
                b.kill()
                fighter.apply_damage(20)
    ...
    if stage == START:
        ...
        display_stats(screen, score, level, high_score, fighter.shield)
